...
function autoWrapLatex(text) {
    ...
    const textPatterns = [
        /^[A-Z][a-z]+(?:[ ][A-Z][a-z]+)*$/,                    // 영어 단어
        /^[A-Z][a-z]+(?:[ ][A-Z][a-z]+)*[.!?]?$/,              // 영어 문장
        /^[A-Za-z0-9\s,.'"!?\-]+$/,                            // 일반 문장
        /^[0-9]+(?:\.[0-9]+)?\s*(?:km|m|kg|mi|ft|in|cm|mm|해리|킬로미터)s?$/, // 숫자+단위
        /^[A-Za-z]+\s+[A-Za-z]+\s+[A-Za-z]+/,                  // 3개 이상 단어
        /^(?:to|for|of|with|from|by|in|at|on|and|or|but|nor|for|yet|so)\s/i, // 전치사 시작
    ];
    ...
    const mathPatterns = [
        ...
        /\\left\([^)]*\\right\)/,
        /\\{.*?\\}/,
        /[a-zA-Z]\^\{?[0-9a-zA-Z]+\}?(?:\s|$)/,  // 변수 제곱 (x^2)
        /[a-zA-Z]_\{[0-9a-zA-Z]+\}/,              // 변수 아래첨자 (x_1)
        /[0-9]+\^\{?[0-9]+\}?/,                   // 숫자 제곱 (2^3)
        /²/,                                      // ² 제곱符号
        /³/,                                      // ³ 세제곱符号
        /[0-9]²/,                                 // 숫자²
