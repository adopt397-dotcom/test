...
function renderLearnPanel(q, displayAnswer) {
  if (!isLearnRevealed(currentIndex)) {
    return '<div class="learn-panel">' +
      '<button type="button" class="learn-reveal-btn">👁 Show Answer & Explanation</button>' +
      '<div class="learn-panel-note">No answer is required in Learn mode.</div>' +
      '</div>';
  }

  var answerLetter = getAnswerLetter(displayAnswer);
  return '<div class="learn-panel revealed">' +
    '<div class="learn-correct-answer">Correct Answer: ' + escapeHtml(answerLetter) + '</div>' +
    '<div class="learn-panel-note">Read the explanation below, then continue.</div>' +
    '</div>';
}
...
function renderSubjectiveQuestion(q, answered, headerText, passageHtml) {
  ...
  var revealLearn = currentMode === 'learn' && isLearnRevealed(currentIndex);
...
function showExplanation(force) {
  ...
  if (currentMode === 'learn' && isLearnRevealed(currentIndex)) {
    var learnAnswer = getAnswerLetter(parseInt(q.answer, 10));
    DOM.explanationText.innerHTML =
      '<div class="learn-correct-answer">Correct Answer: ' + escapeHtml(learnAnswer) + '</div>' +
      renderExplanationLanguageBlock(q);
...
function renderCurrentQuestion() {
  ...
  for (var idx = 0; idx < validKeys.length; idx++) {
    ...
    if (currentMode === 'learn') {
      cls += ' learn-choice';
      if (showLearnAnswer) cls += ' disabled';
      if (showLearnAnswer && isCorrectChoice) cls += ' correct';
  ...
  var learnButton = DOM.questionContainer.querySelector('.learn-reveal-btn');
  if (learnButton) {
